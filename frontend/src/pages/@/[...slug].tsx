...
export default function Page({
  ...
  const { isFollowing, toggle: toggleFollow, loading: followLoading } = useFollow({
  ...
  return (
    <div>
      {profile?.profile_banner ? (
        ...
        <div className="h-32 w-full bg-gray-200 sm:h-48 lg:h-64 dark:bg-dark-second" aria-hidden />
      )}
      <div className="mx-auto max-w-5xl px-3 pb-20 sm:px-6 lg:px-8">
        <div className="relative z-10 -mt-12">
          ...
          <div className="flex items-end justify-between">
            <div className="flex">
              {profile?.profile_picture ? (
                <Image
                  width={512}
                  height={512}
                  className="dark:bg-dark-bg dark:border-dark-border h-20 w-20 rounded-full border-4 border-white bg-white object-cover sm:h-32 sm:w-32 dark:border-dark-main"
                  src={mediaUrl(profile.profile_picture)}
                  alt=""
                />
              ) : (
                <div className="dark:bg-dark-bg dark:border-dark-border flex h-20 w-20 items-center justify-center rounded-full border-4 border-white bg-gray-200 text-2xl text-gray-500 sm:h-32 sm:w-32 dark:border-dark-main" aria-hidden>
            ...
            <div className="flex items-center gap-2 pb-1">
              {isOwner ? (
                <Link
                  href="/profile"
                  className="rounded-full border border-gray-300 px-4 py-1.5 text-sm font-semibold text-gray-900 transition-colors hover:bg-gray-100 dark:border-dark-border dark:text-dark-txt dark:hover:bg-dark-second"
                >
                  Editar perfil
                </Link>
              ) : isAuthenticated ? (
                <button
                  ...
                  className={`rounded-full px-5 py-1.5 text-sm font-semibold transition-colors disabled:opacity-50 ${
                    isFollowing
                      ? 'border border-gray-300 text-gray-900 hover:border-red-300 hover:text-red-600 dark:border-dark-border dark:text-dark-txt'
          ...
          <div className="mt-3">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-4 text-sm text-gray-500 dark:text-dark-txt-secondary">
              <span><strong className="text-gray-900 dark:text-dark-txt">{postsCount ?? 0}</strong> posts</span>
              <span><strong className="text-gray-900 dark:text-dark-txt">{followersCount ?? 0}</strong> seguidores</span>
              <span><strong className="text-gray-900 dark:text-dark-txt">{followingCount ?? 0}</strong> siguiendo</span>
            </div>
          </div>
              ...
              if (item.href && item.href.trim() !== '') {
                return (
                  <Link
                    ...
                    key={item?.name}
                    href={item?.href}
                    className="rounded-full p-1.5 text-gray-500 transition hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-dark-second dark:hover:text-dark-txt"
                  >
                    {item?.icon}
                  </Link>
                );
        ...
        {sanitizedBio && (
          <div className="relative mt-4 pb-2">
            <p
              className="text-sm text-gray-700 dark:text-dark-txt-secondary"
              dangerouslySetInnerHTML={{ __html: isExpand ? sanitizedBio : biographyPreview }}
            />
            {sanitizedBio.length > 600 && (
              <>
                {!isExpand && (
                  <div className="absolute bottom-0 h-16 w-full bg-gradient-to-t from-white dark:from-dark-main" />
                )}
                <button
